...
'''class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = self.dataframe.drop(columns=['image']).iloc[idx].values.astype('float32')
...
if __name__ == "__main__":
    ...
    save_dir = os.path.abspath('D:/Shubham/results/FixmulticlassRSmicroResults')
    ...
    #New Changes
    # Reshape inputs
    '''X_train_np = np.transpose(X_train_np, (0, 3, 1, 2))  # (batch_size, channels, height, width)
    X_pool = np.transpose(X_pool, (0, 3, 1, 2))
    X_test_np = np.transpose(X_test_np, (0, 3, 1, 2))
    X_val_np = np.transpose(X_val_np, (0, 3, 1, 2))

    # Squeeze labels
    y_train_np = y_train_np.squeeze()  # (batch_size,)
    y_pool = y_pool.squeeze()
    y_test_np = y_test_np.squeeze()
    y_val_np = y_val_np.squeeze()

    # Convert to torch tensors
    X_train_initial = torch.tensor(X_train_initial_np, dtype=torch.float32)
    y_train_initial = torch.tensor(y_train_initial_np, dtype=torch.long)'''
    ...
    y_train_np = convert_one_hot_to_labels(y_train_np)
    y_pool_np = convert_one_hot_to_labels(y_pool_np)
    y_val_np = convert_one_hot_to_labels(y_val_np)
    y_test_np = convert_one_hot_to_labels(y_test_np)'''
    ...
    for i in range(no_of_iterations):
        if i == 0:
            ...
            '''cumulative_sample_names.extend(initial_sample_names)
        ...
        if i != 0 and i < no_of_iterations - 1:
            ...
            if train_size > 0:
                ...
                '''added_sample_indices = np.where(np.isin(X_pool_np, X_add))[0]  # Indices of the newly added samples in X_pool_np
                new_sample_names = multiclass_train_df.iloc[added_sample_indices, 0].tolist()  # Get names based on the indices
                cumulative_sample_names.extend(new_sample_names)'''
            ...
            if X_pool_np.shape[0] > 0:
                X_train_initial_np = np.concatenate((X_train_initial_np, X_pool_np), axis=0)
                y_train_initial_np = np.concatenate((y_train_initial_np, y_pool_np), axis=0)
                # Log the names of all remaining samples
                # Log the names of all remaining samples from the pool
                '''remaining_sample_indices = np.where(np.isin(X_pool_np, X_pool_np))[0]  # Get all remaining sample indices
                remaining_sample_names = multiclass_train_df.iloc[remaining_sample_indices, 0].tolist()  # Get names based on the remaining indices
                cumulative_sample_names.extend(remaining_sample_names)

                with open(samples_log_file, mode='a', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow([i + 1, cumulative_sample_names]) '''                                                                                                                
                print(f"Final iteration: Added the remaining {X_pool_np.shape[0]} samples to the training set.")
                X_pool_np, y_pool_np = np.array([]), np.array([])  # Empty the pool after adding the samples
        ...
        checkpoint_path = f"D:/Shubham/results/FixmulticlassRSmicroResults/model_checkpoint_iteration_{i}.pt"
