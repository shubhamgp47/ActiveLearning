...
'''class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # One-hot encode the label using PyTorch
        label = int(self.dataframe.iloc[idx, 1])
...
'''X_initial, y_initial = CustomDataset(initial_df, image_dir, transform)
X_train, y_train = CustomDataset(multiclass_train_df, image_dir, transform)
X_pool, y_pool = CustomDataset(pool_df, image_dir, transform)
X_val, y_val = CustomDataset(multiclass_val_df, image_dir, transform)
X_test, y_test = CustomDataset(multiclass_test_df, image_dir, transform)'''
...
# Function to concatenate batches from DataLoader
def concatenate_batches(loader):
    images, labels = [], []
    for batch in loader:
...
# Concatenate batches to form complete datasets
X_initial, y_initial = concatenate_batches(initial_loader)
X_train, y_train = concatenate_batches(train_loader)
X_pool, y_pool = concatenate_batches(pool_loader)
X_val, y_val = concatenate_batches(val_loader)
X_test, y_test = concatenate_batches(test_loader)

X_initial_np = X_initial
y_initial_np = y_initial
X_train_np = X_train
y_train_np = y_train
X_pool_np = X_pool
y_pool_np = y_pool
...
save_dir = os.path.abspath('D:/Shubham/results/Multiclass01/DinoSmall/ActiveLearning/ParamsFromMultilabel/uncertainty_sampling/Run3_s44/EX/verify_shape')
...
#print("First val label _np:",y_val[0])'''
print("First val label _np:",y_val_np[0])
# Scoring metrics
...
pre_f1 = f1_score(y_test, learner.predict(X_test), average='micro')
pre_acc = learner.score(X_test, y_test)
...
for i in range(n_queries):
    ...
    X_pool = torch.cat((X_pool[:query_index], X_pool[query_index + 1:]), dim=0)
    y_pool = torch.cat((y_pool[:query_index], y_pool[query_index + 1:]), dim=0)
    print(f"Number of samples in pool after training and deleting samples: {X_pool.shape[0]}")
