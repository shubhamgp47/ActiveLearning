...
each iteration have already been chosen by a Margin Sampling Multi-Class
Active Learning process. 

Specifically, we:
1. Initialize our model using the images labeled as Iteration=1 in the CSV 
   (produced by the multiclass margin sampling approach).
...
N_QUERIES = 13
BEST_F1_SCORE = 0
SELECTED_IMAGES_CSV_PATH = 'ActiveLearning/Multiclass/DinoS/margin_sampling_seed42'
FILENAME = "AL_margin_sampling_results_for_multilabel_classification_s42.pickle"
SAVE_PATH = 'ActiveLearning/Multilabel_from_multiclass/DinoS/margin_sampling_seed42_test'
...
def get_images_and_labels_for_iteration(
    ...
    lists pre-selected images (Margin Sampling).
...
print("Selected images DataFrame (Margin Sampling):")
...
for i in range(N_QUERIES):
    print(f"\nQuery {i + 1}: Using images from iteration={i + 2} (margin sampling).")
