...
POWER = 1
NO_OF_ITERATIONS = 14
FILENAME = "random_sampling_results_for_multilabel_classification_s51.pickle"
SPECIFIC_PATH = 'RandomSampling/Multiclass/DinoS/seed_51'
...
class CustomDINONormModel(nn.Module):
    ...
    Methods:
        __init__(dino_model, num_classes): Initializes the model with
        the given DINO model and number of classes.
...
dinoS_model = torch.hub.load(
    'facebookresearch/dinov2', 'dinov2_vits14', pretrained=True)

model = CustomDINONormModel(dinoS_model, num_classes=8).to(DEVICE)
