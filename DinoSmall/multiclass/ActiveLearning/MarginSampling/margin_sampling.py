...
dropout = float(config['multiclass_DinoS_Parameters_9']['dropout'])
layer_freeze = config['multiclass_DinoS_Parameters_9']['layer_freeze']
criterion = eval(f"nn.{config['multiclass_DinoS_Parameters_9']['criterion']}")
optimizer = eval(f"optim.{config['multiclass_DinoS_Parameters_9']['optimizer']}")
lr = float(config['multiclass_DinoS_Parameters_9']['lr'])
patience = int(config['multiclass_DinoS_Parameters_9']['patience'])
step_size = int(config['multiclass_DinoS_Parameters_9']['step_size'])
gamma = float(config['multiclass_DinoS_Parameters_9']['gamma'])

POWER = 1
N_QUERIES = 13
FILENAME = "AL_margin_sampling_results_for_multiclass_classification_s51.pickle"
SPECIFIC_PATH = 'ActiveLearning/Multiclass/DinoS/margin_sampling_seed51'
...
'''for x, df in enumerate(list_data_frame):
    labels = []
    for y, row in tqdm(df.iterrows(), total=df.shape[0]):
        if row['multi-label_double_winding'] == 0:
            if row['multi-label_gap'] == 0:
                if row['multi-label_crossing'] == 0:
                    labels.append('0')
                else:
                    labels.append('1')
            else:
...
class CustomDINONormModel(nn.Module):
    """
    A custom neural network model that integrates a DINO model 
    with additional classification layers.

    Attributes:
        dino_model (nn.Module): The pre-trained DINO model used for feature extraction.
        classifier (nn.Sequential): A sequential container of layers for classification.

    Methods:
        __init__(dino_model, num_classes): Initializes the model with the given DINO model and number of classes.
    ...
    def forward(self, x):
        ...
        Returns:
            torch.Tensor: Output tensor after passing through the DINO model and classifier.
        """
...
dinoL_model = torch.hub.load(
    'facebookresearch/dinov2', 'dinov2_vits14', pretrained=True)

model = CustomDINONormModel(dinoL_model, num_classes=8).to(DEVICE)
