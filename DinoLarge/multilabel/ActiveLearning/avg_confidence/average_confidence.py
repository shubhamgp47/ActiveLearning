...
SPECIFIC_PATH = 'ActiveLearning/Multilabel/DinoL/average_confidence_seed42'
...
dinoL_model = torch.hub.load('facebookresearch/dinov2', 'dinov2_vitl14', pretrained=True)
...
for i in range(N_QUERIES):
    ...
    if i != 12:
        ...
        '''if isinstance(query_index, list):
            query_index = np.hstack(query_index)
            query_index = np.array(query_index).flatten()
            query_index = query_index[:n_instances]'''
        query_index = np.unique(query_index)
