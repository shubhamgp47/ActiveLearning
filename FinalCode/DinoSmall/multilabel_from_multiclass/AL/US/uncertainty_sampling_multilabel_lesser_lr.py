...
each iteration have already been chosen by a Uncertainty Sampling Multi-Class
Active Learning process. 

Specifically, we:
1. Initialize our model using the images labeled as Iteration=1 in the CSV 
   (produced by the multiclass uncertainty sampling approach).
...
N_QUERIES = 13
BEST_F1_SCORE = 0
SELECTED_IMAGES_CSV_PATH = 'ActiveLearning/Multiclass/DinoS/uncertainty_sampling_seed42'
FILENAME = "AL_uncertainty_sampling_results_for_multilabel_classification_s42_test.pickle"
SAVE_PATH = 'ActiveLearning/Multilabel_from_multiclass/DinoS/uncertainty_sampling_seed42_test'
...
def get_images_and_labels_for_iteration(
    ...
    lists pre-selected images (Uncertainty Sampling).
    ...
    """
    matching_rows = selected_images_df[selected_images_df["Iteration"] == iteration]
    if matching_rows.empty:
        raise ValueError(f"No images found for Iteration {iteration}")

    image_list_str = matching_rows["Sample_Names"].values[0]
...
print("Selected images DataFrame (Uncertainty Sampling):")
...
for i in range(N_QUERIES):
    print(f"\nQuery {i + 1}: Using images from iteration={i + 2} (uncertainty sampling).")

    X_query, y_query = get_images_and_labels_for_iteration(
        selected_images_df=selected_images_df,
        iteration=i + 2,
        image_dir=image_dir,
        label_df=train_df,
    ...
    learner.teach(X_query.numpy(), y_query.numpy())

    logits = learner.predict(X_test_np)
