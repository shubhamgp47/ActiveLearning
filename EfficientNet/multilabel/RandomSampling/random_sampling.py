...
dropout = float(config['multilabel_EffNet_Parameters_1']['dropout'])
layer_freeze = config['multilabel_EffNet_Parameters_1']['layer_freeze']
criterion = eval(f"nn.{config['multilabel_EffNet_Parameters_1']['criterion']}")
batch_size = int(config['multilabel_EffNet_Parameters_1']['batch_size'])
optimizer = eval(f"optim.{config['multilabel_EffNet_Parameters_1']['optimizer']}")
lr=float(config['multilabel_EffNet_Parameters_1']['lr'])
patience = int(config['multilabel_EffNet_Parameters_1']['patience'])
fc_units = int(config['multilabel_EffNet_Parameters_1']['fc_units'])

DEVICE = "cuda:0" if torch.cuda.is_available() else "cpu"
FILENAME = "random_sampling_results_for_multilabel_classification.pickle"
NO_OF_ITERATIONS = 14
SPECIFIC_PATH = 'RandomSampling/Multilabel/EffNet/seed_43_config'
...
missing_images = [
    img for img in predefined_image_names if img not in train_df['image'].values]
if missing_images:
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = self.dataframe.drop(
            columns=['image', 'binary_NOK']).iloc[idx].values.astype('float32')
...
class CustomEfficientNetModel(nn.Module):
    """
    A custom model that integrates a pre-trained EfficientNet model with
    additional classification layers and input shape handling.
    """
    def __init__(self, layer_freeze_upto, fc_unit, dropout_rate, num_classes):
        """
        Initializes the CustomEfficientNetModel with a pre-trained EfficientNet model and a classifier.
        ...
        Args:
            ...
            layer_freeze_upto (str, optional): Layer name up to which parameters should be frozen.
        """
        super(CustomEfficientNetModel, self).__init__()

        # Load pre-trained EfficientNet
        self.efficient_net = models.efficientnet_v2_l(weights=models.EfficientNet_V2_L_Weights.IMAGENET1K_V1)
        
        # Freeze layers up to the specified one
        self.freeze_layers(layer_freeze_upto)

        # Define classifier
        num_ftrs = self.efficient_net.classifier[1].in_features
        self.classifier = nn.Sequential(
            nn.Linear(num_ftrs, fc_unit),
            nn.ReLU(),
            nn.Dropout(dropout_rate),
            nn.Linear(fc_unit, num_classes)
        )
    ...
    def forward(self, x):
        ...
        x = self.efficient_net.avgpool(x)
        x = torch.flatten(x, 1)
    ...
    def freeze_layers(self, layer_name):
        """
        Freezes the layers of the EfficientNet model up to the specified layer.

        Args:
            layer_name (str, optional): The name of the layer up to which parameters should be frozen.
        """
        cutoff_reached = False
        for name, param in self.efficient_net.named_parameters():
...
model = CustomEfficientNetModel(layer_freeze, fc_units, dropout, num_classes=3).to(DEVICE)
...
print(
    f"X_initial_np: {X_initial_np.shape}, y_initial_np: {y_initial_np.shape}")
...
classifier = NeuralNetClassifier(
    ...
    device=DEVICE,
    callbacks=[train_f1, valid_f1, csv_logger, es, cp],
    verbose=1
)
    ...
    for i in range(NO_OF_ITERATIONS):
        if i == 0:
            ...
            if n_instances > 0:
                ...
                '''print(f'Shape of X_add is {x_add.shape}')
                print(f'Shape of y_add is {y_add.shape}')'''
                '''if x_add.size > 0:
                    x_train_initial_np = np.concatenate(
                        (X_train_initial_np, x_add), axis=0)
                    y_train_initial_np_param = np.concatenate(
                        (y_train_initial_np, y_add), axis=0)'''
                if x_add.size > 0:
                    x_train_initial_np = np.concatenate(
                        (x_train_initial_np, x_add), axis=0)
                    y_train_initial_np_param = np.concatenate(
