...
def objective(trial):
    ...
    momentum_term = 0 if optimizer_name != 'SGD' else trial.suggest_float('momentum_term', 0.0, 0.9)
    num_epochs = 100
    ...
    dino_repo_path = Path('/home/woody/iwfa/iwfa044h/CleanLab_Test/ActiveLearningApproaches/dino-main03')
    sys.path.insert(0, str(dino_repo_path))
    from vision_transformer import vit_small
    dino_model = vit_small()
    model_state = torch.load(os.path.join(dino_repo_path, "dino_deitsmall16_pretrain03.pth"), map_location="cpu")
    ...
    for epoch in range(num_epochs):
        ...
        if val_f1 > best_val_f1:
            ...
            torch.save(model.state_dict(), 'best_model03.pth')
        else:
...
study = optuna.create_study(
    study_name='optuna_study03',
    direction='maximize',
    storage='sqlite:///optuna_study03.db',
    load_if_exists=True,
...
best_model.load_state_dict(torch.load('best_model03.pth'))
...
pd.DataFrame(all_scores).to_csv("all_scores_DinoV2_XS_5_trials_learn_new_labels03.csv")