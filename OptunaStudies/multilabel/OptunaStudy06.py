...
def objective(trial):
    ...
    dino_repo_path = Path('/home/woody/iwfa/iwfa044h/CleanLab_Test/ActiveLearningApproaches/dino-main06')
    ...
    #model_state = torch.load("/home/woody/iwfa/iwfa044h/CleanLab_Test/ActiveLearningApproaches/DinoV2Large/dinov2_vitl14_pretrain.pth", map_location="cpu")
    ...
    dino_model = torch.hub.load(local_model_path, 'dinov2_vits14', source='local')
    model = CustomDINONormModel(dino_model)
    local_model_path = '/home/hpc/iwfa/iwfa044h/.cache/torch/hub/facebookresearch_dinov2_main'
    dino_model = torch.hub.load(local_model_path, 'dinov2_vitl14', source='local')
    model_state = torch.hub.load('facebookresearch/dinov2', 'dinov2_vitl14')
    dino_model.load_state_dict(model_state, strict=False)'''
    #end
    ...
    for epoch in range(num_epochs):
        ...
        if val_f1 > best_val_f1:
            ...
            torch.save(model.state_dict(), 'best_model06.pth')
        else:
...
study = optuna.create_study(
    study_name='optuna_study06',
    direction='maximize',
    storage='sqlite:///optuna_study06.db',
    load_if_exists=True,
...
best_model.load_state_dict(torch.load('best_model06.pth'))
...
pd.DataFrame(all_scores).to_csv("all_scores_DinoV2_XS_5_trials_learn_new_labels06.csv")