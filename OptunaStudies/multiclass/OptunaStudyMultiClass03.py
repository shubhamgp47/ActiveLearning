...
def define_model(dropout, fc_units, fc_units_2):
    ...
    model = CustomDINONormModel(dino_model, num_classes, dropout, fc_units, fc_units_2).to(device)
    return model

# Optuna study setup and optimization
study = optuna.create_study(
    study_name='optuna_study10multiclass03',
    direction='maximize',
    storage='sqlite:///optuna_study10multiclass03.db',
    load_if_exists=True
)
...
def objective(trial):
    ...
    model = define_model(dropout, fc_units, fc_units_2)
    model.to(device)
...
'''
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in train_loader:
            ...
            preds = torch.argmax(probabilities, dim=1).detach()  # Use argmax to get predicted class
            all_labels.extend(labels.cpu().numpy())
            all_predictions.extend(preds.cpu().numpy())
