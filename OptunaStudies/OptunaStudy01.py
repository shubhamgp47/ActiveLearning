...
def objective(trial):
    ...
    layer_freeze = trial.suggest_int('layer_freeze', 0, 311)
    ...
    momentum_term = None if optimizer_name == 'Adam' else trial.suggest_float('momentum_term', 0.0, 1.0)
    ...
    # Load DINO model
    dino_repo_path = Path('/home/woody/iwfa/iwfa044h/CleanLab_Test/ActiveLearningApproaches/dino-main01')
    sys.path.insert(0, str(dino_repo_path))
    from vision_transformer import vit_small
    dino_model = vit_small()
    model_state = torch.load(os.path.join(dino_repo_path, "dino_deitsmall16_pretrain01.pth"), map_location="cpu")
    ...
    for epoch in range(num_epochs):
        ...
        if val_f1 > best_val_f1:
            best_val_f1 = val_f1
            # Save the best model
            torch.save(model.state_dict(), 'best_model01.pth')
...
storage='sqlite:///optuna_study01.db'
study = optuna.create_study(study_name='optuna_study01', direction='maximize', storage=storage, load_if_exists=True)
...
# Load DINO model
dino_repo_path = Path('/home/woody/iwfa/iwfa045h/CleanLab_Test/ActiveLearningApproaches/dino-main01')
sys.path.insert(0, str(dino_repo_path))
from vision_transformer import vit_small

# Load the best model
best_model = CustomDINONormModel(vit_small(), best_trial.params['FC_units'], best_trial.params['dropout_rate'])
best_model.load_state_dict(torch.load('best_model01.pth'))
best_model.to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))

# Save all scores
test_dataset = CustomImageDataset(test_df, image_dir, y_columns, get_transform(best_trial.params['image_size']))
...
pd.DataFrame(all_scores).to_csv("all_scores_DinoV2_XS_5_trials_learn_new_labels01.csv")
...
plot.write_image("{}.svg".format("hyperparameters_importance"))             # Saving hyperparameter importance as vector graphic
plot.show()

plot = optuna.visualization.plot_slice(study)
plot.write_image("{}.svg".format("slice_plot"))                            # Saving slice plot as vector graphic
plot.show()'''